...
?>
  <div class="htmlmail-body">
    ...
    <p>
      These event(s) were just added and take place before the next weekly email is scheduled to be sent.
    </p>
    <p>
      You're receiving this email because you checked the box to receive urgent emails. You can change this by clicking on the Change Subscription link below.
    </p>


    <?php foreach ($params['events'] as $event): ?>
    <table class="climate-event" style="margin-bottom: 40px;width: 600px;">
      <tr>
        <td class="climate-event__start-time" style="font-weight:bold;font-size: 18px;"><?php print date('F j @ g:ia', strtotime($event->getEventStartTime()));?></td>
      </tr>
      <tr>
        <td class="climate-event__title" style="font-weight:bold;font-size: 14px;"><?php print $event->getEventTitle(); ?> ( <?php print $event->getAddressObject()->getCity(); ?> )</td>
      </tr>
      <tr>
        <td class="climate-event__description" style="width:400px;font-size: 13px;"><?php print $event->getEventDescription(); ?></td>
      </tr>
      <tr>
        <td class="climate-event__address" style="padding: 5px 0 10px 0;font-size: 13px;">
          <?php print $event->getEventAddressForHTMLEmail(); ?>

        </td>
      </tr>
      <tr>
        <td class="climate-event__types" style="width: 600px;font-size: 13px;">
          <?php print $event->getEventTypeNamesForHTMLEmail(); ?>
        </td>
      </tr>
    </table>
    <?php endforeach; ?>
    ...
    <div>
      <?php print $params['unsubscribe_link']; ?>
    </div>
    <div>
      <?php print $params['change_subscription_link']; ?>
    </div>
  </div>
<?php if ($debug):
